...
def main():
	...
	survey_ids = {
		...
		'2018_DWI_Monday': 'SV_3IBWgXXN1CMQLaJ',
		'2018_DWI_Tuesday': 'SV_dgmeg9bpHwRWKl7',
		'2018_DWI_Wednesday': 'SV_d7psWJmDQiu6pdX'
...
def normalize_crosstab(values, program, year, day):
	...
	session_regex = re.compile('\[([^\s]+) Session\]')
	...
	for i in range(3, len(values)):
		...
		if program == 'DWJ' or program == 'DWI' or program == 'DWAU':
			...
			for datacol in range(numDataCols):
				...
				if m is not None:
				...
				elif re.search('What worked well.*about.*(session|case discussion)', question_text) is not None:
					question = 'What worked well about this session?'
					...
					newRow.append(year)
					newRow.append(day)
					newRow.append(current_type)
					newRow.append(current_session)
					newRow.append(current_leader)
					newRow.append(question)
					newRow.append(session_response)
					sessionRows.append(newRow)

				elif re.search('What would you.*change.*about.*(session|case discussion)', question_text) is not None:
					question = 'What would you have liked to change about this session?'
					session_response = response
					newRow.append(role)
					newRow.append(team)
					newRow.append(program)
				...
				#Add to overallRows if it is
					...
					for regex in question_search_dict[program]:
						...
						if m is not None:
							...
							if question_category is 'Team Norms' or question_category is 'Group Norms' or question_category is 'Components' or question_category is 'Objectives':
